...
class SecurityPlane {
  ...
  begin(req, res, pathname) {
    ...
    const isLoopback = /^(::1|127\.0\.0\.1|::ffff:127\.0\.0\.1)$/.test(address);
    const localRuntimeRead = isLoopback && req.method === 'GET' && pathname.startsWith('/api/');
    const limit = isLogin ? this.config.loginRateLimit : localRuntimeRead ? this.config.apiRateLimit * 8 : this.config.apiRateLimit;
