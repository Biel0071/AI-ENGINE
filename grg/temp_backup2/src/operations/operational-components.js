...
function createOperationalComponents(context, tenantId) {
  ...
  return [
    ...
    component('mission-kernel', ['job-engine', 'event-store'], stateProbe((state) => ({ missions: scoped(state.missions, tenantId).length, active: scoped(state.missions, tenantId).filter((item) => ['RUNNING', 'AWAITING_APPROVAL', 'PAUSED'].includes(item.status)).length, events: scoped(state.missionEvents, tenantId).length })), true),
    component('event-store', ['state-store'], stateProbe((state) => ({ events: scoped(state.domainEvents, tenantId).length })), true),
    ...
    component('process-resources', ['metrics'], async () => { const memory = process.memoryUsage(); const cpu = process.cpuUsage(); return { ok: true, version: process.version, evidence: { rssBytes: memory.rss, heapUsedBytes: memory.heapUsed, heapTotalBytes: memory.heapTotal, externalBytes: memory.external, cpuUserMicros: cpu.user, cpuSystemMicros: cpu.system } }; }, true),
