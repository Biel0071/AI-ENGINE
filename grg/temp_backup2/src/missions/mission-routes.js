function handleMissionRoutes(req, res, url, app, sendJson) {
  if (req.method === 'GET') {
    ...
    if (url.pathname === '/api/workers') {
      if (!app.workerRegistry) {
        const activeJobs = app.jobWorker?.activeJobs?.size || 0;
        const total = app.jobWorker?.maxConcurrent || 0;
        sendJson(res, 200, {
          workers: app.jobWorker ? [{
            id: 'fenix-dev-cloud-worker',
            name: 'FENIX DEV CLOUD worker',
            status: app.jobWorker.intervalId ? 'ACTIVE' : 'STOPPED',
            activeJobs,
            capacity: total
          }] : [],
          source: 'jobWorker'
