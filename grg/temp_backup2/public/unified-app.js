...
async function refreshAll() {
  ...
  try {
    const activeView = String(location.hash.slice(1) || 'city').split('?')[0] || 'city';
...
function renderCity() {
    if (window.drawCity) { 
      window.globalCityState = state.data; 
      window.state = window.state || {};
      window.state.agentStates = state.data.swarm?.agents || state.data.agents || {}; 
      window.drawCity();
    }
}
...
async function runChat(message) {
  ...
  pending.textContent = 'Criando mission job no Runtime FENIX...';
  $('chatLog').appendChild(pending);
  try {
    const res = await api('/dev/tasks', {
      method: 'POST',
      body: JSON.stringify({
        prompt: value,
    ...
    if (window.openJobInspector) window.openJobInspector(res.job?.id || res.jobId, value);
    pending.remove();
    bubble(`Job criado no runtime: ${res.job?.id || res.jobId || 'sem id publicado'}\nStatus: ${res.job?.status || 'QUEUED'}`, 'bot');
    await refreshAll();
  } catch (error) {
    pending.remove();
...
async function loadFs(path = '') {
  try {
    ...
    if ($('fsList')) $('fsList').innerHTML = groupedFsHtml(items) || row('fs', 'vazio', 'EMPTY');
    document.querySelectorAll('#fsList [data-fs-path]').forEach((el) => {
      el.addEventListener('click', () => {
        const filePath = el.dataset.fsPath || '';
        if (el.dataset.fsDir === 'true') { $('fsPath').value = filePath; loadFs(filePath); }
...
function renderFsBreadcrumb(path = '') {
  ...
  const crumbs = parts.length ? parts.map((part, index) => ({ label: index === 0 && /^[A-Za-z]:$/.test(part) ? 'FÊNIX' : part, path: parts.slice(0, index + 1).join('/') })) : [{ label: 'PROJECT', path: '' }];
  crumb.innerHTML = crumbs.map((part, index) => `<button type="button" data-fs-crumb="${index}">${esc(part.label)}</button>`).join('<span>/</span>');
  crumb.querySelectorAll('[data-fs-crumb]').forEach((button) => button.addEventListener('click', () => loadFs(crumbs[Number(button.dataset.fsCrumb)]?.path || '')));
}

function groupedFsHtml(items) {
  if (!items.length) return '';
  if (items.length < 18) return items.map(fsItemHtml).join('');
  const groups = new Map();
  items.forEach((item) => {
    const group = fsGroupFor(item);
    if (!groups.has(group)) groups.set(group, []);
    groups.get(group).push(item);
  });
  return Array.from(groups.entries()).map(([label, groupItems]) => `<div class="fs-group-heading">${esc(label)}<span>${groupItems.length}</span></div>${groupItems.map(fsItemHtml).join('')}`).join('');
}

function fsItemHtml(item) {
  return `<button class="row fs-tree-row" type="button" data-fs-path="${esc(item.path)}" data-fs-dir="${item.isDirectory ? 'true' : 'false'}">
    <b>${esc(item.name)}</b><small>${esc(item.path)}</small><span>${item.isDirectory ? 'DIR' : 'FILE'}</span>
  </button>`;
}

function fsGroupFor(item) {
  const text = `${item.name || ''} ${item.path || ''}`.toLowerCase();
  if (/frontend|public|component|page|style|css|html|archive|clients|crm/.test(text)) return 'apps / frontend';
  if (/core|kernel|agent|orchestrator|memory|rag|skill|mind|cognitive|ai-os/.test(text)) return 'core';
  ...
  if (/memory|pattern|knowledge|dna|data/.test(text)) return 'memory';
  if (/test|spec|playwright|e2e|smoke/.test(text)) return 'tests';
  if (/patch|scratch|temp|log|backup|bkp|artifact/.test(text)) return 'workspace artifacts';
  return 'workspace';
}
...
function init() {
  ...
  window.addEventListener('hashchange', () => showView(location.hash.slice(1) || 'city', false));
  window.addEventListener('hashchange', () => refreshAll());
  showView(location.hash.slice(1) || 'city', false);
  bubble('FENIX AI City carregada como home. O navegador e control plane; o runtime executa missoes e jobs reais.');
  refreshAll();
...
// --- AI CITY 3D CANVAS & INTERACTION ----------------------------------
  window.initCityCanvas = function() {
    ...
    let tick = 0;
      ...
      function stableAgentSeed(value) {
        let hash = 0;
        String(value || 'fenix-agent').split('').forEach((ch) => { hash = ((hash << 5) - hash + ch.charCodeAt(0)) | 0; });
        return Math.abs(hash);
      }
      function stableAgentCoord(seed, salt, span) {
        const next = (seed * (salt * 97 + 17) + salt * 7919) % 10000;
        return (next / 10000 - 0.5) * span;
      }
      function mapRealAgentsToVisual() {
         ...
         agentKeys.forEach((key, i) => {
            ...
            if (ra.status !== 'ACTIVE' && ra.status !== 'RUNNING') return;
             ...
             if (!va) {
               const seed = stableAgentSeed(key);
               va = {
                 key,
                 x: stableAgentCoord(seed, 1, 400),
                 y: stableAgentCoord(seed, 2, 300),
                 targetX: stableAgentCoord(seed, 3, 400),
                 targetY: stableAgentCoord(seed, 4, 300),
                 speed: 0.0008 + ((seed % 600) / 1000000),
                 avatar: 'AG',
                 role: ra.role || key,
                 fullName: key,
                 color: '#38bdf8',
...
window.esc = esc;
  ...
  // --- GENERIC VIEW SWITCHING FOR ALL TABS ---
  document.querySelectorAll('button.nav-item[data-view]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      document.querySelectorAll('button.nav-item').forEach(nav => nav.classList.remove('active'));
      ...
      if (!view) {
        ...
        view.innerHTML = `<div class="empty-state"><h2>${viewId.toUpperCase()} View</h2><p>Content for ${viewId} view placeholder.</p></div>`;
  ...
  // ─── LIVE RUNTIME BRIDGE ─────────────────────────────────────────────────
  // unified-app.js reacts to live events from live-runtime.js
  // cockpit-app.js handles the cockpit panel directly
  document.addEventListener('fenix-live', (e) => {
    const { type, ...data } = e.detail || {};

    if (type === 'snapshot') {
      const q = data.queue || {};
      const kpiEl = document.getElementById('kpiAgents');
      if (kpiEl) kpiEl.textContent = `${q.running || 0} running`;
      const queueEl = document.getElementById('liveQueueBadge');
      if (queueEl) queueEl.textContent = `${q.queued || 0} queued`;
    }

    if (type === 'runtime.heartbeat') {
      const uptimeEl = document.getElementById('runtimeUptime');
      const u = data.uptime || window.FENIX?.live?.uptime || 0;
      if (uptimeEl) {
    ...
    // Stream all events to the eventStream panel
    const eventStream = document.getElementById('eventStream');
    if (eventStream && type && type !== 'runtime.heartbeat' && type !== 'pong') {
      const line = document.createElement('div');
      line.className = 'event-log';
      line.innerHTML = `[${esc(type)}] <strong>${esc(JSON.stringify(data).slice(0, 100))}</strong>`;
      eventStream.insertBefore(line, eventStream.firstChild);
