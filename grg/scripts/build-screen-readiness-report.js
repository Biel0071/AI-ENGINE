...
const rows = (map.screens || []).map(screen => ({
  ...
  navigation: navByScreen[screen.name]?.ok === true ? 'PASS' : (navigation.blocked ? 'BLOCKED_AUTH' : 'NOT_RUN'),
...
const report = { generatedAt: new Date().toISOString(), policy: 'evidence-separated-no-mock', domains: Object.keys(map.domains || {}).length, screens: rows.length, navigationSummary: navigation.summary || {}, contractSummary: contracts.summary || {}, rows };
fs.writeFileSync(out, JSON.stringify(report, null, 2));
console.log(JSON.stringify({ domains: report.domains, screens: report.screens, output: out, navigationBlocked: Boolean(navigation.blocked) }, null, 2));
