...
const { FenixWSBridge } = require('./eventing/fenix-ws-bridge');
...
const { handleCloudRoutes } = require('./api/cloud-routes');
const { ProjectRegistry } = require('./projects/project-registry');
const { JobQueue } = require('./execution/job-queue');
const { JobWorker } = require('./execution/job-worker');
const { AgentAvatarRegistry } = require('./agents/agent-avatar-registry');
const { ProceduralLearningEngine } = require('./memory/procedural-learning-engine');
...
async function start(port = Number(process.env.PORT || 4400), options = {}) {
  ...
  // FÊNIX DEV CLOUD COMPONENTS
  const fenixDataDir = path.join(__dirname, '..', '..', '.data');
  app.projectRegistry = new ProjectRegistry(path.join(fenixDataDir, 'projects.json'));
  app.jobQueue = new JobQueue(path.join(fenixDataDir, 'job-queue.json'));
  app.agentAvatarRegistry = new AgentAvatarRegistry(path.join(fenixDataDir, 'agent-avatars.json'));
  app.proceduralLearning = new ProceduralLearningEngine(path.join(fenixDataDir, 'procedural-learning.json'));
  ...
  app.jobWorker = new JobWorker(app);
  app.jobWorker.start(3000); // Check every 3 seconds
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (req.method === 'GET' && url.pathname === '/runtime/snapshot') {
        try {
          const snapshot = app.wsBridge ? await app.wsBridge.buildSnapshot(app) : { type: 'runtime.snapshot', payload: { status: 'BRIDGE_NOT_READY' } };
          return sendJson(res, 200, snapshot.payload, requestId);
        } catch (err) {
          return sendJson(res, 500, { error: 'snapshot failed', detail: err.message }, requestId);
        }
      }
      ...
      if (req.method === 'POST' && url.pathname === '/api/test/event') {
        ...
        if (app.bus) {
          await app.bus.emit(body.type || 'system.validation.tick', body.payload || body);
        }
      ...
      // FÊNIX DEV CLOUD API
      const cloudHandled = await handleCloudRoutes(req, res, url, app, sendJson, (r, s, e) => sendJson(r, s, { error: e }));
      ...
      if (!url.pathname.startsWith('/api/')) return await serveStatic(url.pathname, res, app);
      ...
      if (req.method === 'GET' && url.pathname === '/api/learning/procedural') return sendJson(res, 200, app.proceduralLearning.summary(), requestId);
      if (req.method === 'POST' && url.pathname === '/api/learning/reuse') return sendJson(res, 200, app.proceduralLearning.findReusable(await readJson(req)), requestId);
  ...
  // ─── FÊNIX LIVE RUNTIME — WebSocket Bridge ──────────────────────────────
  const wss = new WebSocketServer({ noServer: true });
  
  // Bridge conecta o EventBus interno ao WS: heartbeat, ring-buffer, replay, sanitize
  const wsBridge = new FenixWSBridge({ bus: app.bus, wss });
  wsBridge.start();
  app.wsBridge = wsBridge;

  // Aceita /events E /runtime (compatibilidade com fenix-bootstrap.js)
  server.on('upgrade', (request, socket, head) => {
    const url = (request.url || '').split('?')[0];
    if (url === '/events' || url === '/runtime') {
  ...
  // JobWorker emite para app.eventBus — alinhar com app.bus
  if (app.jobWorker) {
    app.jobWorker.app.eventBus = app.bus;
  }
...
async function serveStatic(pathname, res, app = null) {
  ...
  if (type === 'text/html' && rel === 'index.html' && app?.wsBridge) {
    try {
      const snapshot = await app.wsBridge.buildSnapshot(app);
      const payload = JSON.stringify(bootSnapshotPayload(snapshot.payload)).replace(/</g, '\\u003c');
      const script = `<script type="application/json" id="fenix-initial-snapshot">${payload}</script>`;
      body = Buffer.from(String(body).replace('</head>', `${script}\n</head>`), 'utf8');
    } catch {
      // The app still boots from the client-side runtime fallback if the bridge is not ready.
    }
  }
  res.end(body);
}

function bootSnapshotPayload(snapshot = {}) {
  const slimJob = (job) => ({
    id: job.id,
    type: job.type,
    status: job.status,
    ...
    filesChanged: Array.isArray(job.filesChanged) ? job.filesChanged.slice(0, 8) : [],
    logs: Array.isArray(job.logs) ? job.logs.slice(-6) : [],
    microtasks: Array.isArray(job.microtasks) ? job.microtasks.slice(0, 6) : []
  });
  return {
    ...
    jobs: Array.isArray(snapshot.jobs) ? snapshot.jobs.slice(0, 80).map(slimJob) : [],
    missions: Array.isArray(snapshot.missions) ? snapshot.missions.slice(0, 20) : [],
    operationalTwin: snapshot.operationalTwin
  };
}
