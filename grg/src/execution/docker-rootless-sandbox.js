...
class DockerRootlessSandbox {
  constructor({ workspaceRoot, dockerHost, exec = execFileAsync, enforceRootless = true, tempRoot = os.tmpdir(), cpuMode = 'quota' } = {}) {
    if (!workspaceRoot) throw new ValidationError('sandbox requires an authorized workspace root');
    if (!['quota', 'process-time'].includes(cpuMode)) throw new ValidationError('invalid sandbox CPU mode');
    this.workspaceRoot = path.resolve(workspaceRoot); this.dockerHost = dockerHost || process.env.DOCKER_HOST || ''; this.exec = exec; this.tempRoot = path.resolve(tempRoot); this.productionSafe = enforceRootless; this.cpuMode = cpuMode;
  ...
  async run({ executionId, tool, argv, workspacePath, limits, network = 'none', environment = {} }) {
    ...
    const containerName = `fenix-sbx-${String(executionId).slice(0, 12)}`;
    let started = false;
    try {
      if (workspacePath) { const source = this.#workspace(workspacePath); fs.cpSync(source, workdir, { recursive: true, dereference: false }); }
      // The rootless daemon runs as another host user and must traverse the
      // random staging directory to bind-mount its workspace.
      fs.chmodSync(temp, 0o711);
      fs.chmodSync(workdir, 0o755);
      const environmentFile = writeEnvironmentFile(temp, environment);
      const args = dockerArgs({ executionId, tool, argv, workdir, limits, network, environmentFile, cpuMode: this.cpuMode });
      started = true;
      ...
      return { exitCode: 0, stdout: String(result.stdout || ''), stderr: String(result.stderr || ''), sandbox: { driver: 'docker-rootless', network, image: tool.image, limits, cpuMode: this.cpuMode } };
    } catch (error) {
      if (started) {
        try { await this.exec('docker', ['rm', '-f', containerName], { windowsHide: true, timeout: 5_000, env: { ...process.env, DOCKER_HOST: this.dockerHost } }); } catch { /* --rm may already have removed it */ }
      }
...
function dockerArgs({ executionId, tool, argv, workdir, limits, network, environmentFile = null, cpuMode = 'quota' }) {
  if (!['quota', 'process-time'].includes(cpuMode)) throw new ValidationError('invalid sandbox CPU mode');
  const cpuSeconds = Math.max(1, Math.ceil((limits.timeoutMs / 1000) * (limits.cpuUnits / 1000)));
  const args = ['run', '--rm', '--name', `fenix-sbx-${String(executionId).slice(0, 12)}`, '--label', `grg.execution=${executionId}`, '--read-only', '--cap-drop', 'ALL', '--security-opt', 'no-new-privileges', '--pids-limit', String(limits.pids), '--memory', `${limits.memoryMb}m`, ...(cpuMode === 'quota' ? ['--cpus', String(limits.cpuUnits / 1000)] : ['--ulimit', `cpu=${cpuSeconds}:${cpuSeconds + 1}`]), '--network', network, '--tmpfs', '/tmp:rw,noexec,nosuid,size=64m', '--mount', `type=bind,src=${workdir},dst=/workspace`, '--workdir', '/workspace'];
