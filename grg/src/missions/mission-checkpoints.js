...
const CHECKPOINT_EVENTS = new Set([
  ...
  'mission.repair.created', 'mission.step.reconciled',
]);
...
class MissionCheckpointsService {
  ...
  async capture(event, metadata = {}) {
    ...
    const resolvedMissionId = missionId || mission?.missionId || ('job:' + (jobId || 'standalone'));
...
function classify(type) { if (type === 'mission.created') return 'AUTO'; if (type.includes('repair') || type.includes('reconciled') || type.includes('cancel') || type.includes('pause')) return 'RECOVERY'; if (type.includes('succeeded') || type === 'mission.completed') return 'POST_VALIDATION'; return 'AUTO'; }
