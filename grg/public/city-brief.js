(() => {
  ...
  function render() {
    ...
    const status = el('p', 'fcb-status', state.busy ? 'Sincronizando dados…' : state.error ? `Atualização parcial: ${state.error}` : state.updatedAt ? `Medido às ${new Date(state.updatedAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}` : 'Aguardando dados'); status.setAttribute('role', 'status');
  ...
  window.addEventListener('fenix:viewchanged', (event) => { if (event.detail?.viewId === 'city') refresh(); });
