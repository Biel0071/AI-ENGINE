      ...
      <Card title="Lista de campanhas" subtitle="Execucao, status e fila">
        <div className="space-y-3">
          {campaigns.map((campaign) => (
            <div key={campaign.id} className="rounded-lg border border-borderSoft bg-panelSoft p-3">
              <div className="flex items-center justify-between">
                <div>
              ...
              <p className="mt-2 text-xs text-slate-400">Fila: {campaign.queue?.sent ?? 0}/{campaign.queue?.total ?? 0}</p>
            </div>
          ))}
        </div>
      </Card>
              ...
              {contacts.map((contact) => {
                ...
                return (
                  <button
                    ...
                    className={`block w-full rounded-lg border px-3 py-2 text-left text-sm ${active ? 'border-accent bg-accent/20' : 'border-borderSoft bg-panelSoft'}`}
                  >
